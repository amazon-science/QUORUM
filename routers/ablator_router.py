...
class AblatorRouter:
    ...
    def route(self, llm_predictions, human_labels, confidence_scores, routing_strategy="minimize", **kwargs):
        ...
        confidence_scores = kwargs['difficulty_features'].mean(axis=1)
        #confidence_scores = np.random.rand(kwargs['difficulty_features'].shape[0])
                ...
                for i in range(n):
                    ...
                    if self.money_used + cost_of_action > kwargs['money_budget']:
                        if action == 1 and self.money_used + llm_cost <= kwargs['money_budget']:
                            ...
                            action = -1
                            print(f"Ablator - Stopped annotating at sample {i}")
                    ...
                    if action == 1:
                        ...
                        try:
                            output_set.append(llm_predictions[i]+1)
                        except TypeError:
                            output_set.append("-1")
